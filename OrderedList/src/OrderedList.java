...
/**
 * Maintains a list of ints sorted in ascending order. New ints may be
 * inserted into the list, and ints may be removed from the list
 */
public class OrderedList
{
   ...
   public void insert(int arg)
   {
      ...
      // find index of the first value > arg (if there is one) and
      // insert arg at that index.  If there is no greater value in the array
      // or the array is empty, arg is inserted as the new last element 
      ...
      for (int i = 0 ; i < count && !found ; i++)
      {
         if (sorted[i] > arg)        // found it!
         {
            ...
            /**************************************************
            * Note to students: lines 67-76 are the guts of   *
            * ArrayList method "add(i,object)" after first    *
            * resizing the array if it was full (lines 41-46) *
            **************************************************/
   ...
   /*****************************************************
   * Note to students: this next method is exactly what *
   * happens in ArrayList method "remove(object)"       *
   *****************************************************/
   
   /**
    * Searches the list for an int to be removed. If found, 
    * removes it and returns true; if not found, returns false.
    * If the list has duplicates, only the first is removed
   ...
   {     
      ...
      /***************************************************
      * Note to students: lines 127-132 are exactly what *
      * happens in ArrayList method "remove(index)"      *
      ***************************************************/
